...
public enum Opcode: OpcodeType, CustomStringConvertible {
    ...
    public var textualDescription: String {
        ...
        case .SkipIfEqualValue(x: let x, value: let value):
            return "Skips the next instruction if V\(hex(x)) equals \(value)"
        case .SkipIfNotEqualValue(x: let x, value: let value):
            return "Skips the next instruction if V\(hex(x)) doesn't equal \(value)"
        case .SkipIfEqualRegister(x: let x, y: let y):
            return "Skips the next instruction if V\(hex(x)) equals V\(hex(y))"
        case .SetValue(x: let x, value: let value):
        ...
        case .SkipIfNotEqualRegister(x: let x, y: let y):
            return "Skips the next instruction if V\(hex(x)) doesn't equal V\(hex(y))"
        case .SetIndex(address: let addr):
        ...
        case .SkipIfKeyPressed(x: let x):
            return "Skips the next instruction if the key stored in V\(hex(x)) is pressed"
        case .SkipIfKeyNotPressed(x: let x):
            return "Skips the next instruction if the key stored in V\(hex(x)) is not pressed"
        case .StoreDelayTimer(x: let x):
    ...
    public init?(rawOpcode: UInt16) {
        ...
        let nib3 = UInt8((instruction & 0x00F0) >> 4)
        let nib4 = UInt8(instruction & 0x000F)
        ...
        case (0x0, 0x0, 0xE, 0xE):
            self = .Return
        case (0x1, _, _, _):
            self = .JumpAbsolute(address: instruction & 0xFFF)
        case (0x2, _, _, _):
            self = .CallSubroutine(address: instruction & 0xFFF)
        case (0x3, let x, _, _):
            self = .SkipIfEqualValue(x: x, value: UInt8(instruction & 0xFF))
        case (0x4, let x, _, _):
            self = .SkipIfNotEqualValue(x: x, value: UInt8(instruction & 0xFF))
        case (0x5, let x, let y, 0x0):
            self = .SkipIfEqualRegister(x: x, y: y)
        case (0x6, let x, _, _):
            self = .SetValue(x: x, value: UInt8(instruction & 0xFF))
        case (0x7, let x, _, _):
            self = .AddValue(x: x, value: UInt8(instruction & 0xFF))
        ...
        case (0xA, _, _, _):
            self = .SetIndex(address: instruction & 0xFFF)
        case (0xB, _, _, _):
            self = .JumpRelative(address: instruction & 0xFFF)
        case (0xC, let x, _, _):
            self = .AndRandom(x: x, value: UInt8(instruction & 0xFF))
